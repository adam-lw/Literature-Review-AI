...
def create_agent_project(
    ...
    """Creates an agent-mode project: free-text description only, no initial search queries or
    embedding_run_id (those get filled in once the scoping conversation produces a scope and
    the search_review stage starts running searches). Mirrors what AgentLocalStore.createProject
    used to do client-side in localStorage, now persisted server-side.
    """
    with ENGINE.begin() as conn:
        row = (
            conn.execute(
                text("""
                INSERT INTO app.projects (project_title, description, inclusion_criteria, project_mode)
                VALUES (:title, :description, :inclusion_criteria, 'agent')
                RETURNING project_id
            """),
                {
...
def get_scope(conversation_id: str) -> dict[str, Any] | None:
    with ENGINE.connect() as conn:
        row = (
            conn.execute(
                text("""
                SELECT scope_id, conversation_id, content, created_at, updated_at
