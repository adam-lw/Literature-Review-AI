...
-- A project's initial free-text research description is not stored here - it lives solely as
-- the scoping conversation's own first app.messages turn (see AgentProjectWorkspace.jsx's
-- handleStartPhase), since that's already where it ends up once scoping starts. Keeping a
-- second copy on this table was pure redundancy.
...
-- "scope_title"/"scope_description" are a best-effort, LLM-generated identity for the scope
-- (distinct from the project's own project_title), written once when the scoping phase is
-- marked completed - see AgentProjectWorkspace.jsx's handleContinuePhase and agent_service's
-- generate-scope-description endpoint. Nullable: generation is fire-and-forget from the
-- frontend, so a scope can legitimately sit with these still NULL.
